...
n_epochs = 1
n_dipoles = 3
X = np.array([np.hstack(epochs[str(i)].get_data()[1:1 + n_epochs])
...
dim = 20
X = np.array([np.linalg.svd(x, full_matrices=False)[2][:dim] for x in X])

# Get sources with multiviewICA
W_m, S_multiview = multiviewica(X, verbose=True, random_state=0)
W_g, S_group = groupica(X, random_state=0)
W_p, S_perm = permica(X, random_state=0)

S_m = [np.dot(W, x) for W, x in zip(W_m, X)]
S_g = [np.dot(W, x) for W, x in zip(W_g, X)]
S_p = [np.dot(W, x) for W, x in zip(W_p, X)]
# Compute true source
...
for i in range(n_dipoles):
    plt.figure()
    for S, name in zip([S_g, S_p, S_m],
                       ['Group ICA', 'Perm ICA', 'Multiview ICA']):
